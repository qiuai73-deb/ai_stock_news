...
class DeepAnalyzer:
    ...
    def _init_client(self):
        ...
        try:
            ...
            # 优先从环境变量读取，其次从配置读取（与 importance_analyzer 保持一致）
            env_key = os.getenv("DEEPSEEK_API_KEY") or os.getenv("OPENROUTER_API_KEY")

            # 优先获取 active_provider，如果没有设置，优先判断 deepseek 节点
            self.provider = ai_cfg.get("active_provider")
            if not self.provider:
                if "deepseek" in ai_cfg and (ai_cfg["deepseek"].get("api_key") or env_key):
                    self.provider = "deepseek"
                else:
                    self.provider = "openrouter"

            provider_cfg = ai_cfg.get(self.provider, {})
            api_key = env_key or provider_cfg.get("api_key")

            # 统一的占位符判定（与 importance_analyzer 一致，
            # 避免把字面量占位符 DEEPSEEK_API_KEY 当成真实 key 发送导致 401）
            PLACEHOLDERS = [
                "DEEPSEEK_API_KEY", "YOUR_API_KEY", "YOUR_DEEPSEEK_API_KEY",
                "OPENROUTER_API_KEY", ""
            ]
            if not api_key or api_key in PLACEHOLDERS or "YOUR_" in str(api_key).upper():
                logger.warning(f"未配置有效的 {self.provider} API密钥，深度分析将使用模拟模式")
                self.client = None
                return

            # 优先使用环境变量中的 base_url（与 GitHub Actions 工作流注入的
            # DEEPSEEK_BASE_URL / OPENROUTER_BASE_URL 一致），其次取配置
            env_base_url = (
                os.getenv("DEEPSEEK_BASE_URL") if self.provider == "deepseek"
                else os.getenv("OPENROUTER_BASE_URL")
            )
            base_url = env_base_url or provider_cfg.get("base_url")
            if not base_url:
    ...
    def _analyze_with_ai_self_search(self, news_item: NewsItem) -> DeepAnalysisResult:
        ...
        try:
            ...
            result = DeepAnalysisResult(
                ...
                model_used=self.config.get("ai_analysis", {}).get(getattr(self, "provider", "openrouter"), {}).get("model", "deepseek-reasoner")
            )
...
深度分析报告："""
    ...
    def _call_ai_model(self, prompt: str) -> str:
        ...
        try:
            ...
            # DeepSeek 各模型输出上限约为 8K，超大值会触发 400 错误，需收敛
            deep_max_tokens = self.deep_config.get("max_tokens", 8000)
            safe_max_tokens = max(1, min(deep_max_tokens, 8000))

            # 构造请求参数；推理模型（reasoner）不支持 temperature，必须跳过
            kwargs = {
                "model": model,
            ...
            if "reasoner" not in model.lower():
                kwargs["temperature"] = provider_cfg.get("temperature", 0.1)
