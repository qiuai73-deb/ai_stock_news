...
class AIAnalyzer:
    ...
    def _setup_client(self):
        ...
        if self.provider == "openrouter":
            ...
            env_key_names = ["OPENROUTER_API_KEY", "DEEPSEEK_API_KEY"]
        ...
        # 3) 占位符检测：避免拿占位符去打真实 API 导致 401
        placeholders = {
            "", "DEEPSEEK_API_KEY", "YOUR_API_KEY", "YOUR_DEEPSEEK_API_KEY",
            "OPENROUTER_API_KEY", "YOUR_OPENROUTER_API_KEY",
        }
        if api_key in placeholders or "YOUR_" in api_key.upper():
            error_msg = (
                f"未配置有效的 {provider_name} API 密钥（当前为占位符 {api_key!r}）。\n"
                f"请二选一：① 设置环境变量 {env_key_names[0]}=sk-xxx；"
                f"② 在 {config_path_info} 填入真实 key。"
            )
        ...
        try:
            # base_url：优先环境变量，其次配置，最后默认
            base_url = (os.environ.get(env_base_url_name) or "").strip() or ai_config.get(
...
"""
    ...
    def _call_ai_api(self, prompt: str) -> str:
        ...
        # 推理模型（reasoner）不支持 temperature 与 response_format(JSON模式)
        is_reasoner = "reasoner" in str(model).lower()
        ...
        try:
            # 组装请求参数；推理模型需跳过 temperature / response_format
            req_kwargs = dict(
                model=model,
            ...
            if is_reasoner:
                logger.info("检测到推理模型(reasoner)，跳过 temperature / response_format 参数")
            else:
