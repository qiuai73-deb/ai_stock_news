...
from .eastmoney_news import EastMoneyNewsCollector
...
class NewsCollector:
    ...
    def collect_all_news(self) -> List[NewsItem]:
        """
        根据配置收集财经新闻（支持多数据源：chinanews / eastmoney）

        Returns:
            List[NewsItem]: 收集到的新闻列表
        """
        news_config = self.config.get("news_collection", {})
        sources_cfg = news_config.get("sources", {})
        rss_feeds = sources_cfg.get("rss_feeds", [])

        # 每条新闻抓取上限：优先用 news_collection.max_items，其次 rss_feeds[0].max_items
        max_items = news_config.get("max_items") or (
            rss_feeds[0].get("max_items", 50) if rss_feeds else 50
        )

        # 启用的数据源列表（顺序即优先级）；默认东方财富 WEB 爬取（config 缺失时也能用）
        active_sources = news_config.get("active_sources") or ["eastmoney"]
        logger.info(f"开始收集财经新闻，启用数据源: {active_sources}（上限 {max_items} 条）")

        all_news: List[NewsItem] = []

        # 1) 东方财富（WEB 爬取模式：列表页 + 文章页）
        if "eastmoney" in active_sources:
            try:
                em_cfg = news_config.get("eastmoney", {})
                em_max = em_cfg.get("max_items", max_items)
                em_mode = em_cfg.get("mode", "web")
                em_pages = em_cfg.get("pages")
                collector = EastMoneyNewsCollector(
                    max_items=em_max, mode=em_mode, pages=em_pages
                )
                raw_news = collector.fetch_news(max_items=em_max)
                for news_data in raw_news:
                    try:
                        ...
                        logger.error(f"转换东方财富新闻数据失败: {e}")
                        self.stats["errors"] += 1
                logger.info(f"从东方财富收集到 {len(raw_news)} 条新闻")
            except Exception as e:
                logger.error(f"东方财富收集失败: {e}")
                self.stats["errors"] += 1

        # 2) 中国新闻网 RSS（保留作为可选数据源 / 回退）
        if "chinanews" in active_sources:
            try:
                ...
                for news_data in raw_news:
                    try:
                        ...
                        self.stats["errors"] += 1
                logger.info(f"从中国新闻网RSS收集到 {len(raw_news)} 条新闻")
            except Exception as e:
